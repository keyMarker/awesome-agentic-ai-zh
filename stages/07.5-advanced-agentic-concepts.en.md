...
### 📐 Prompts for drawing your own concept diagrams

The ASCII diagrams above are one rendering. The 4 prompts below let you ask Claude / Codex to draw your own version in **mermaid** (GitHub renders this natively) or **PNG** (via an image-gen MCP):

#### Prompt 6 — Why → What → How flow diagram (mermaid)

```
Generate a mermaid flowchart representing a "Why → What → How" concept map with the following content:

- Left column (Why): 6 production agent pain points
  1. Context 200k cap / multi-agent context overflow
  2. Agent can't read its own codebase / docs
  3. Multi-agent desync, multiple "truths"
  4. Random drift / review misses it
  5. Agent ships PRs faster than human QA
  6. Jumping to multi-agent from day 1

- Middle column (What): the matching principle(s)
  1 → Progressive Disclosure + Memory persistence
  2 → Legibility + Tool Doc / ACI
  3 → System of Record
  4 → Taste Invariants + Transparency
  5 → Throughput Changes Merge Philosophy
  6 → Simplicity (Anthropic)

- Right column (How): concrete tool / mechanism
  Skills references/, CLAUDE.md @-import, .ai/<task>.md brief
  AGENTS.md (100 lines), poka-yoke tool design
  docs/ + .coord/, shared-memory skill
  acceptance-gate preset YAMLs, evaluator-optimizer
  mandatory preset, LLM-as-judge, human spot-check
  start with basic LLM call

Requirements:
- flowchart LR (left-to-right)
- Use subgraphs to group nodes by WHY / WHAT / HOW
- Node labels under 30 chars
- Arrows go Why → What → How
- Output a mermaid code block that GitHub renders inline
```

#### Prompt 7 — 5 principles × 8 components mapping matrix (mermaid + table)

```
Generate the mapping matrix of "OpenAI's 5 harness engineering principles × Stage 7's 8 core harness components":
...
Columns (8 components):
1. Agent Loop / 2. Tool Registry / 3. Context Manager / 4. Retry / 5. Sandbox / 6. Observability / 7. Eval / 8. Cost-Latency

Mark each cell:
- ✓★ = primary lever (the principle's main effect on that component)
- ✓ = applies but not primary
- (empty) = does not apply

Give both outputs:
(a) A clean markdown table
(b) A mermaid heatmap / quadrantChart-style visualization if mermaid supports it

End with 2-3 sentences of insight: which component is hit by the most principles, and why.
```

#### Prompt 8 — Four-layer work boundary stack diagram (mermaid)

```
Draw a 4-layer work-boundary stack diagram in mermaid:

Top-down (autonomy from high to low):
- **Service layer** (highest autonomy): multi-agent supervisor / planner-executor / PAR loop (Reflexion / Self-Refine) / Agent-as-Judge / self-organizing teams
- **Repo layer**: shared memory (.coord/memory.yml) / vector DB / RAG retrieval / episodic memory
- **Config layer**: plan.yml / context_policy / budget / acceptance presets / banned phrasing / autonomy gradient
- **Types layer** (lowest autonomy): task brief format / result.json schema / tool definitions / MCP primitives / prompt template signatures

Each layer should be a subgraph listing 3-5 example items.
Between layers, draw a dashed line labeled "work boundary N" (N = 1/2/3).
On the left side, draw a vertical arrow labeled "Agent autonomy ↑" to indicate higher layers = more autonomy.

Output mermaid graph TB (top-bottom) + one sentence per boundary explaining its meaning.
```

#### Prompt 9 — Generate a PNG concept diagram (via image-gen MCP)

```
Use the image-gen MCP tool to create a 16:9 banner-style concept diagram:

Title: "Why Harness Engineering Principles Matter"

Layout: 3 columns horizontal
- Left column WHY: 6 production agent failure modes (icons + short text)
- Middle column WHAT: 5 OpenAI principles + 3 Anthropic-extra principles (iconified)
- Right column HOW: matching tools (CLAUDE.md / Skills / acceptance-gate icons)
- Arrows connecting columns

Style:
- Clean tech-blog aesthetic (reference Anthropic engineering blog)
- 2-3 colors max, sans-serif typography
- Generous whitespace, not packed
- Suitable as a README hero image

Message: convey "agent design has predictable failure modes; harness engineering principles solve them; concrete tools implement the principles."
```

→ **Suggested usage**: for a README banner or slide deck use Prompt 9 (PNG); for inline rendering in markdown use Prompts 6/7/8 (mermaid). **Mermaid renders natively on GitHub — no PNG files to maintain**.
