...
上面的 ASCII 图只是其中一种画法。下面 4 个 prompt 让你叫 Claude / Codex 用 **mermaid**（GitHub 直接 render）或 **PNG**（透过 image-gen MCP）画成你自己版本：

#### Prompt 6 — Why → What → How 流程图（mermaid）

```
请用 mermaid flowchart 生成一张「Why → What → How」概念图，内容如下：

- 左栏 (Why)：6 个 production agent 痛点
  1. Context 200k 满 / multi-agent context overflow
  2. Agent 看不懂自家 codebase / docs
  3. 多 agent desync、不同事实版本
  4. 随机 drift / review 漏抓
  5. Agent 写 PR 快、human QA 跟不上
  6. 一上来就堆 multi-agent overkill
...
- 右栏 (How)：具体工具 / 机制
  ...
  AGENTS.md 100 行、poka-yoke tool design
...
要求：
- flowchart LR (左到右)
- 用 subgraph 分 WHY / WHAT / HOW 三组
- 每个 node label < 30 chars
- 箭头从 Why → What → How
- 输出 GitHub 能直接 render 的 mermaid code block
```

#### Prompt 7 — 5 原则 × 8 元件 对照矩阵（mermaid + table）

```
请生成「OpenAI 5 harness engineering 原则 × Stage 7 harness 8 元件」的对照矩阵：
...
每格标记：
- ✓★ = primary lever（这条原则在这个元件的主要作用点）
- ✓ = applies but not primary
- (空) = 不适用

两种输出都给：
(a) 一张干净的 markdown 表格
(b) 一张 mermaid 热力图 / quadrantChart 风格的视觉化（如果 mermaid 支援）

最后加 2-3 句洞察：哪个元件被最多原则打到、为什么。
```

#### Prompt 8 — 四层工作边界 stack 图（mermaid）

```
请用 mermaid 画一张 4 层工作边界（work boundary）stack 图：

从上到下（autonomy 从高到低）：
- **Service layer**（最高自主权）：multi-agent supervisor / planner-executor / PAR loop (Reflexion / Self-Refine) / Agent-as-Judge / self-organizing teams
...
- **Types layer**（最低自主权）：task brief format / result.json schema / tool definitions / MCP primitives / prompt template signatures

每层用 subgraph、列出 3-5 个 example item。
层与层之间画一条 dashed line 标「work boundary N」（N = 1/2/3）。
左侧画一个垂直箭头标「Agent autonomy ↑」表示越上层自主权越高。

输出 mermaid graph TB（top-bottom）+ 一句话解释每个 boundary 的意义。
```

#### Prompt 9 — 生成 PNG 概念图（image-gen MCP）

```
请用 image-gen MCP 工具生成一张 16:9 banner 风格的概念图：

标题：「Why Harness Engineering Principles Matter」

内容：3 columns 横向 layout
- 左栏 WHY：6 个 production agent failure modes（图标 + 短文字）
- 中栏 WHAT：5 OpenAI 原则 + 3 Anthropic 额外原则（icon 化）
- 右栏 HOW：对应工具（CLAUDE.md / Skills / acceptance-gate / etc. icon）
- 栏与栏之间用箭头连接

风格：
- Tech-blog 简洁风（参考 Anthropic engineering blog）
- 2-3 色（最多）、sans-serif 字体
- 留白多、不要塞满
- 适合 README hero image 用

讯息：要传达「agent 设计有可预测的 failure modes、harness 原则解决它们、具体工具落地」。
```

→ **建议用法**：要 README banner / 简报投影片用 Prompt 9（PNG）；要嵌在 markdown 内 inline render 用 Prompt 6/7/8（mermaid）。**Mermaid GitHub 原生 render、不用维护 PNG 档案**。
